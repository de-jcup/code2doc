...
public class Transformer {
	 ...
	 * <li>enumeration - returns enum.simpleClassName()+"."+name() (when
	 * toString() is same as name()(default) - otherwise a string of
	 * enum.simpleClassName()+"."+name() +":"+toString() will be returned )</li>
	...
	/**
	 * Transforms a given clazz to an instance. The clazz must have a default
	 * constructur and must be public accessible!
	 * 
	 * @param clazz
	 * @return instance
	 * @throws IllegalArgumentException
	 *             - when given clazz cannot be created
	 */
	...
	/**
	 * Transforms given object to a tag - tags are used e.g. to identify
	 * elements
	 * <ol>
	 * <li>null object to "null"</li>
	 ...
	 * 
	 * @param resourcePath
	 *            - path must be suitable for class loading - e.g.
	 * 
	 *            <pre>
	 * &quot;/de/jcup/test/pictures/picture01.png&quot;
	 * </pre>
	 * @return file or <code>null</code> if not transformable/not found
	 */
	public File transformToFile(String resourcePath) {
		/*
		 * Remark: convenience resource handling is only done inside
		 * Element.prepareResourcePath() - not here
		 */
	...
	/**
	 * Transforms given class name to a class
	 * 
	 * @param className
	 * @return class or <code>null</code>
	 */
	public Class<?> transformToClass(String className) {
		if (className == null) {
			return null;
		}
		try {
			...
			return tryInnerClassResolving(className);
			/* ignore - we only return null */
		}
	}

	Class<?> tryInnerClassResolving(String className) {
		String createdInnerClassName = null;
		try {
			createdInnerClassName = buildInnerClassName(className);
			if (createdInnerClassName==null){
				return null;
			}
			Class<?> clazz = Class.forName(createdInnerClassName);
			return clazz;
		} catch (ClassNotFoundException e) {
			if (createdInnerClassName != null) {
				return tryInnerClassResolving(createdInnerClassName);
			}
			return null;
		}
	}
	...
	/**
	 * Builds an inner class name
	 * @param className
	 * @return inner class name or <code>null </code>if no possible
	 */
	String buildInnerClassName(String className) {
		int index = className.lastIndexOf('.');
		if (index == -1) {
			/* no inner classes possible */
			return null;
		}
		String path1 = className.substring(0, index);
		String path2 = className.substring(index+1);
		String createdInnerClassName = path1 + '$' + path2;
		return createdInnerClassName;
	}
}